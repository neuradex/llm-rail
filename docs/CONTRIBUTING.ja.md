...
- **サンプルワークフロー** — プロダクションレベルの複雑なユースケースを示すワークフロー例（マルチステップパイプライン、ポリシー適用、バリデーションチェーン、accumulateパターンなど）
- **エージェント統合** — Claude Code以外のAIエージェントとの統合（OpenAI Codex、Devin、Cursor Agentなど）
- **ベンチマーク** — 現実的なbefore/after比較：実際のタスクでLLM Rail使用の有無によるコスト、精度、完了率の測定
- **`lrail docs` インデキシング** — エージェントが全ファイルをスキャンせずに関連トピックを効率的に発見できるドキュメント検索設計
- **Hooksパターン** — 一般的なニーズに対応する再利用可能なライフサイクルフックレシピ（通知、メトリクス、CIゲートなど）
- **バグ報告・設計フィードバック** — バグ、設計上の欠陥、違和感のある部分を見つけましたか？Issueを開いてください。あらゆるフィードバックを歓迎します。
