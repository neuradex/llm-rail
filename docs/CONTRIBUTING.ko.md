...
- **벤치마크** — 현실적인 before/after 비교: 실제 태스크에서 LLM Rail 사용 유무에 따른 비용, 정확도, 완료율 측정
- **`lrail docs` 인덱싱** — 에이전트가 모든 파일을 스캔하지 않고도 관련 토픽을 효율적으로 찾을 수 있는 문서 검색 설계
- **Hooks 패턴** — 일반적인 요구사항을 위한 재사용 가능한 라이프사이클 훅 레시피 (알림, 메트릭, CI 게이트 등)
- **버그 리포트 & 설계 피드백** — 버그, 설계 결함, 또는 뭔가 어색한 부분을 발견하셨나요? 이슈를 열어주세요. 모든 피드백을 환영합니다.
