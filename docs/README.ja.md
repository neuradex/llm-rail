...
コアメカニズム：**エージェントのすべてのコマンドは`lrail <id> bash '<cmd>'`を経由する必要があります。** この単一の制御ポイントで、ポリシー適用と監査ログが行われます。

```
エージェント → lrail <id> bash 'curl ...' → ポリシーチェック → 監査ログ → 実行（または拒否）
```
...
| ポリシー適用 | 構造的（バイパス不可） | プロンプト依存 |
| WebSearch / WebFetch | 利用不可 | 利用可能 |

### ポリシールール

ワークフローのポリシールールは、bashプロキシの**上で**動作します。`lrail <id> bash`を通過するすべてのコマンドがこのルールで評価されます：
...
3. `WebFetch`/`WebSearch`の代わりに、bashプロキシ経由の`curl`でウェブにアクセスします
