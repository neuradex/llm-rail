...
LLM에는 **최신성 편향(recency bias)**이 있다 — 컨텍스트가 길어질수록 원래 지시를 잊어버린다 ([Lost in the Middle, Liu et al. 2023](https://arxiv.org/abs/2307.03172)). 이게 복잡한 에이전틱 작업의 근본적인 실패 패턴이다.
...
그리고 진행 상황을 추적하는 것은 LLM이 아니라 워크플로우 엔진이기 때문에, **수백 개의 스텝이 있는 워크플로우라도 빠짐없이 전부 실행된다.** 긴 컨텍스트의 LLM 에이전트는 필연적으로 스텝을 빼먹는다. 워크플로우 엔진은 절대 까먹지 않는다.

엔터프라이즈에게 이건 세 가지 핵심 질문에 답한다: **"복잡한 프로세스를 처리할 수 있는가?"** — 엔진이 모든 스텝의 완료를 보장한다. **"통제할 수 있는가?"** — 정책 레일로 가능. **"문제를 추적할 수 있는가?"** — 감사 레일로 가능. 전부 프롬프트 레벨의 약속이 아니라 아키텍처 레벨에서 답한다.
